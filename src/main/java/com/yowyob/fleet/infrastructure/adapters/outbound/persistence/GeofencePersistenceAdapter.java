...
public Flux<GeofenceZone> findByFleetId(UUID fleetId) {
    ...
    public Mono<Void> saveEvent(UUID vehicleId, UUID zoneId, String type) {
        ...
        return eventRepo.save(event)
                .doOnSuccess(e -> log.info("🌍 Event Geofence sauvegardé: {} - Zone: {} - Vehicule: {}", type, zoneId, vehicleId))
                .then();
    }

    // Requête dynamique pour les événements (Binding manuel pour éviter l'injection SQL)
    @Override
    public Flux<GeofenceEventEntity> findEventsWithFilters(UUID vehicleId, UUID zoneId, String type, LocalDate date) {
        ...
        if (vehicleId != null) queryBuilder.append(" AND vehicle_id = :vehicleId");
        if (zoneId != null) queryBuilder.append(" AND zone_id = :zoneId");
        if (type != null) queryBuilder.append(" AND type = :type");
        if (date != null) queryBuilder.append(" AND DATE(timestamp) = :date");
        
        DatabaseClient.GenericExecuteSpec spec = databaseClient.sql(queryBuilder.toString());

        if (vehicleId != null) spec = spec.bind("vehicleId", vehicleId);
        if (zoneId != null) spec = spec.bind("zoneId", zoneId);
        if (type != null) spec = spec.bind("type", type);
        if (date != null) spec = spec.bind("date", date);
    ...
    public Mono<Void> linkZoneToFleetManager(UUID fleetManagerId, UUID zoneId) {
        ...
        return databaseClient.sql(findFleetSql)
                ...
                .switchIfEmpty(Mono.error(new IllegalStateException("Impossible de créer une zone : Vous devez d'abord créer une Flotte.")))
                .flatMap(fleetId -> {
                    ...
                    return databaseClient.sql(updateSql)
                            ...
                            .flatMap(rows -> {
                                if (rows == 0) {
                                    return Mono.error(new RuntimeException("Erreur technique : Zone créée mais introuvable pour la liaison (ID: " + zoneId + ")"));
                                }
                                return Mono.empty();
                            });
                });
